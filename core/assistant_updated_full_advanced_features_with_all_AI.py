...
def listen(self):
    ...
    def handle_user_query(self, query):
        ...
        if "افتح المتصفح" in query or "شوف الأخبار" in query:
            self.speak("جاري فتح المتصفح على الأخبار.")
            webbrowser.open("https://news.google.com/")

        elif "افتح تطبيق" in query:
            app_name = query.replace("افتح تطبيق", "").strip()
            self.speak(f"أقوم الآن بفتح التطبيق: {app_name}")
            try:
                subprocess.Popen(app_name)
            except Exception as e:
                self.speak(f"تعذر فتح التطبيق {app_name}: {e}")

        elif "حرك الماوس" in query or "شوف الشاشة" in query:
            self.speak("جاري تحريك الماوس.")
            pyautogui.moveTo(600, 400, duration=1)

        elif "اكتب برومت" in query and "كيروسور" in query:
            self.speak("ما هو البرومت الذي تود كتابته؟")
            prompt = self.listen()
            with open("cursor_prompt.txt", "w", encoding="utf-8") as f:
                f.write(prompt)
            self.speak("تم حفظ البرومت في ملف كيروسور.")

        elif "خذ لقطة شاشة" in query:
            screenshot = pyautogui.screenshot()
            screenshot.save("screenshot.png")
            self.speak("تم التقاط لقطة الشاشة بنجاح.")

        else:
            self.speak("جاري استخدام الذكاء الاصطناعي لتحليل طلبك.")
