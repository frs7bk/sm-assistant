...
try:
    ...
    print(f"⚠️ تعذر تحميل نظام التكوين المتقدم: {e}")
    ...
    async def initialize(self):
        ...
        try:
            ...
            if settings_available:
                ...
                validation_issues = self.settings.validate_config()
                if validation_issues:
                    self.logger.warning(f"⚠️ مشاكل في التكوين: {', '.join(validation_issues)}")
                else:
                    self.logger.info("✅ تم تحميل نظام التكوين المتقدم بنجاح")
            elif legacy_settings_available:
                self.settings = get_settings()
                if not validate_environment():
                    self.logger.warning("⚠️ بعض الإعدادات غير مكتملة")
                else:
                    self.logger.info("✅ تم تحميل الإعدادات القديمة")
            else:
                self.logger.warning("⚠️ تشغيل بدون نظام إعدادات")
